...
{
	if (m_pDataBuff && memcmp(pData, "IMKH", 4) != 0)
	{
...
int CHikSdkParser2::GetOnePacket(char *pData, J_StreamHeader &streamHeader)
{
	...
	if (m_bIsComplate)
	{
		...
		if (memcmp(m_pOutBuff, H264_SPS_HEAD, 5) == 0 || memcmp(m_pOutBuff, H264_PPS_HEAD, 5) == 0)
		{
			streamHeader.frameType = jo_video_a_frame;
		}
		else
		{
			streamHeader.frameType = (memcmp(m_pOutBuff, H264_I_HEAD, 5) == 0) ? jo_video_i_frame : jo_video_p_frame;
		}
